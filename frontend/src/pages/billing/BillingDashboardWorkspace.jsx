...
import {
  FileText, ShoppingBag, Boxes, Plus, RefreshCw
} from 'lucide-react';
...
export default function BillingDashboardWorkspace() {
  ...
  // Dummy Data to bypass API error and show UI instantly
    ...
    <div className="space-y-6 text-xs">
      ...
      {viewMode === 'quick-links' ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {[
            { title: 'Sale Invoice', path: '/billing/invoices/create?type=sale' },
            { title: 'Purchase Invoice', path: '/billing/invoices/create?type=purchase' },
            { title: 'Quotation', path: '/billing/quotations/create' },
            { title: 'Delivery Challan', path: '/billing/challans/create' },
            { title: 'Proforma Invoice', path: '/billing/proforma/create' },
            { title: 'Purchase Order', path: '/billing/purchase-orders/create' },
            { title: 'Sale Order', path: '/billing/sales-orders/create' },
            { title: 'Inward Payment', path: '/billing/payments/create?type=received' },
          ].map((card, idx) => (
            <button
              key={idx}
