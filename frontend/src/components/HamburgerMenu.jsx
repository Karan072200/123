  ...
  const menuSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Low Stock Alerts', path: '/inventory?filter=low-stock', icon: AlertTriangle },
      ]
    },
    {
    ...
    },
    {
      title: 'REPORTS',
      items: [
        { label: 'Sales & Billing Reports', path: '/reports-ai', icon: BarChart3 },
        { label: 'Business Reports', path: '/reports', icon: FileText },
        { label: 'Profit & Loss Summary', path: '/reports?tab=pnl', icon: PieChart },
        { label: 'GST Reports', path: '/reports?tab=gst', icon: ShieldAlert },
      ]
    },
    {
      ...
      items: [
        ...
        { label: 'AI Accounting Assistant', path: '/reports-ai', icon: Bot },
      ]
    },
    {
      ...
      items: [
        ...
        { label: 'Settings & Profile', path: '/settings', icon: SettingsIcon },
        ...
        <div className="w-screen max-w-md bg-white dark:bg-slate-900 shadow-2xl flex flex-col">
          <div className="p-4 bg-emerald-800 text-white flex items-center justify-between shadow-md">
            <div className="flex items-center space-x-3">
              <img src="/apkamunim-playstore-icon-512.png" alt="Apka Munim" className="w-9 h-9 rounded-lg bg-white p-1" />
              <div>
                <h2 className="font-bold text-lg leading-tight">Apka Munim</h2>
                <p className="text-xs text-emerald-200">{currentUser?.email || 'Accounting Workspace'}</p>
              </div>
            </div>
            <button onClick={onClose} className="p-1.5 rounded-full hover:bg-emerald-700 transition-colors">
              <X className="w-6 h-6" />
            </button>
          </div>
                    ...
                    return (
                      ...
                      >
                        <Icon className={`w-4 h-4 ${
                          ...
                          item.highlight ? 'text-amber-600' : 'text-slate-400'
                        }`} />
