...
if not JWT_SECRET:
    raise RuntimeError(
        "JWT_SECRET is required. Set it in backend/.env to a long random string "
        "(e.g., `openssl rand -hex 32`)."
    )
GOOGLE_CLIENT_ID = os.environ.get("GOOGLE_CLIENT_ID", "").strip()
if not GOOGLE_CLIENT_ID:
    raise RuntimeError(
        "GOOGLE_CLIENT_ID is required. Set it in backend/.env "
...
class AccountUpdateIn(BaseModel):
    ...
    type: Optional[Literal["savings", "current", "cash", "wallet", "credit_card", "emergency", "investment", "other"]] = None
...
async def update_account(account_id: str, body: "AccountUpdateIn", user=Depends(get_current_user)):
    existing = await db.accounts.find_one(
        {"id": account_id, "owner_id": user["current_ledger_id"]}, {"_id": 0}
    )
    if not existing:
    ...
    if updates:
        await db.accounts.update_one(
            {"id": account_id, "owner_id": user["current_ledger_id"]},
            {"$set": updates},
        )
    return {"ok": True}
...
FREE_VOICE_LLM_DAILY_LIMIT = 5


@api.post("/voice/parse-transaction")
@limiter.limit("30/hour")
async def voice_parse_transaction(request: Request, body: VoiceParseIn, user=Depends(get_current_user)):
    """
    Parse spoken/typed text like "500 rupaye zomato pe" into a transaction dict.
    Uses regex first, LLM as fallback (rate-limited + premium/free-tier gated).
    """
    import re
    ...
    if amount is None:
        # Gate LLM fallback: premium bypass, else free-tier daily counter
        pstatus = await _sync_premium_status(user)
        if not pstatus["premium_active"]:
            allowed = await _check_daily_free_limit(
                user, "voiceLlmCount", FREE_VOICE_LLM_DAILY_LIMIT
            )
            if not allowed:
                raise HTTPException(
                    status_code=402,
                    detail=(
                        f"Daily voice AI limit ({FREE_VOICE_LLM_DAILY_LIMIT}) reached. "
                        "Upgrade to Premium for unlimited voice parsing."
                    ),
                )
        # Try LLM fallback
        llm_prompt = (
...
class WarrantyUpdateIn(BaseModel):
...
async def update_warranty(warranty_id: str, body: WarrantyUpdateIn, user=Depends(get_current_user)):
    existing = await db.warranties.find_one(
        {"id": warranty_id, "owner_id": user["current_ledger_id"]}, {"_id": 0}
    )
    if not existing:
...
@api.patch("/kids/{kid_id}")
async def update_kid(kid_id: str, body: KidUpdateIn, user=Depends(get_current_user)):
    existing = await db.kids.find_one(
        {"id": kid_id, "owner_id": user["current_ledger_id"]}, {"_id": 0}
    )
    if not existing:
    ...
    if updates:
        await db.kids.update_one(
            {"id": kid_id, "owner_id": user["current_ledger_id"]},
            {"$set": updates},
        )
    return {"ok": True}
...
class PublicDeleteAccountRequestIn(BaseModel):
    email: EmailStr
    reason: Optional[str] = ""


@api.post("/public/delete-account-request")
@limiter.limit("5/hour")
async def public_delete_account_request(
    request: Request, body: PublicDeleteAccountRequestIn
):
